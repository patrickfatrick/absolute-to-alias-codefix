...
const topLevelFiles = fs
  ...
  .filter(
    filename => !/node_modules/.test(filename) && filename.indexOf(".") !== 0
  )
  .map(filename =>
    filename.replace(/\.[a-z0-9]{1,5}$/i, "").replace(/\./g, "\\.")
  );
...
const requireRegex = new RegExp(
  `require\\((['"])(${topLevelFiles.join("|")})`,
  "g"
);
const jestRegex = new RegExp(
  `jest\\.mock\\((['"])(${topLevelFiles.join("|")})`,
  "g"
);
...
console.log(
  "Testing against",
  chalk.green("import"),
  chalk.yellow(importRegex)
);
console.log(
  "Testing against",
  chalk.green("require"),
  chalk.yellow(requireRegex)
);
    ...
    .forEach(filename => {
      ...
      fs.readFile(filepath, "utf8", (err, data) => {
        ...
        const newData = data
          .replace(
            importRegex,
            (_, p1, p2, p3) => `${p1} from ${p2}${alias}/${p3}`
          )
